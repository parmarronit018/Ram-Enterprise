...
import { fetchProducts, createOrder, decreaseProductQuantity } from "./firebaseUtils";
...
function App() {
    ...
    try {
      ...
      try {
        ...
        if (typeof parsed === "object" && parsed !== null) {
          const name = parsed.name || parsed.email?.split("@")[0] || null;
          if (name) localStorage.setItem("ram_user", name);
  ...
  const [isAdmin, setIsAdmin] = useState(() => localStorage.getItem("ram_is_admin") === "true");
  ...
  const [activeView, setActiveView] = useState(() => {
    // On first load: if admin logged in → admin panel, if user logged in → home, else → login
    ...
    return "login"; // always show login first if not logged in
  });
  ...
  const handlePlaceOrder = async (paymentType) => {
    ...
    try {
      ...
      // Decrease stock quantity for each item in Firestore
      for (const item of cartItems) {
        const pid = item.id || item._id;
        // Only decrease if it's a real Firestore ID (not hardcoded numeric)
        if (pid && !/^\d+$/.test(pid)) {
          await decreaseProductQuantity(pid, 1);
        }
      }
  ...
  const handleLogout = () => {
    ...
    setActiveView("login"); // always go to login on logout
    window.scrollTo({ top: 0, behavior: "smooth" });
  };
